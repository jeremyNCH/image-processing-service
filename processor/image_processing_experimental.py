...
class Film(object):
    ...
    def copy_into(self, source_image, l, t, r, b, target_image, x, y):
        """
        Crop a region from the source image and composite it into the target image at (x, y).
        Returns the modified target image.
        """
    ...
    def move_kernel(self, image_size, source_image, l, t, r, b, target_image, x, y):
        """
        Move a cropped region (kernel) from the source image to the target image.
        Updates the (x, y) position for the next kernel placement.
        Returns updated coordinates and the modified target image.
        """
        if y >= image_size:
            raise ValueError
    ...
    def create_black_reference(self):
        """
        Generate a black reference image by sampling the borders of the negative image.
        Returns the filename of the generated black reference image.
        """
    ...
    def calculate_gamma_correction(self, blackref_image):
        """
        Calculate the gamma correction factor based on the black reference image.
    ...
    def calculate_shift(self, blackref_image, channel):
        """
        Calculate the minimum value (shift) for a given color channel in the black reference image.
        Returns the shift value for the specified channel.
        """
    ...
    def calculate_rgb_shifts(self, blackref_image):
        """
        Calculate the shift values for the red, green, and blue channels in the black reference image.
        Returns a tuple of (r_shift, g_shift, b_shift).
        """
    ...
    def check_black_and_white(self, blackref_image):
        """
        Determine if the black reference image is black and white (grayscale).
    ...
    def analyze_black_reference(self):
        """
        Analyze the black reference image to extract color and exposure information.
        Sets internal state for color correction and black/white detection.
        """
    ...
    def calculate_black_point(self, spinner):
        """
        Determines and sets the black point reference for the negative image.
        
        This function chooses the method for black point calculation based on user input:
        - If a preset is specified and recognized, it loads all preset values for color correction and black/white detection.
        - If a custom black reference image is provided, it uses that image for analysis.
        - Otherwise, it generates a black reference image by sampling the specified borders of the negative.
        
        The function logs which method is used, which borders are sampled (if applicable), and the resulting color/exposure statistics.
        It updates the Film instance's internal state with all relevant correction parameters for subsequent processing steps.
        """
        if (
    ...
    def calculate_color_gammas(self, negative_image):
        """
        Calculate gamma correction factors for the green and blue channels based on the negative image.
        Returns a tuple (gamma_g, gamma_b).
        """
    ...
    def calculate_color_factors(self, negative_image):
        """
        Calculate color scaling factors for the green and blue channels based on the negative image.
        Returns a tuple (factor_g, factor_b).
        """
        with Image(negative_image) as negative_image_copy:
            negative_image_copy.resize(1, 1)
    ...
    def adjust_and_save_negative(self):
        """
        Apply all color corrections and adjustments to the negative image and save the result as a positive image.
        """
        with Image(filename=self.negative, resolution=300) as negative_image_container:
            with Image(negative_image_container.sequence[0]) as negative_image:

                if not self.is_black_and_white and self.black_color:
    ...
    def invert(self):
        """
        Main entry point: inverts the negative image to a positive, handling all steps and error reporting.
        """
